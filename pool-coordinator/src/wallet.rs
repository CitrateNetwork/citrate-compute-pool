...
const ENV_KEYSTORE_PATH: &str = "CITRATE_POOL_KEYSTORE_PATH";
const ENV_KEYSTORE_PASSPHRASE: &str = "CITRATE_POOL_KEYSTORE_PASSPHRASE";
const ENV_PRIVATE_KEY_HEX: &str = "CITRATE_POOL_PRIVATE_KEY_HEX";
...
impl Wallet {
    ...
    pub fn sign_eip1559(&self, tx: &Eip1559Tx) -> Result<Vec<u8>, CoordinatorError> {
...
mod tests {
    ...
    fn loads_from_keystore_with_correct_passphrase() {
        ...
        let w = Wallet::from_keystore(path.to_str().unwrap(), "hunter2").expect("unlock");
        assert_eq!(
            w.address(),
            "0x7e5f4552091a69125d5dfcb7b8c2659029395bdf"
                .parse::<H160>()
    ...
    fn rejects_unsupported_version() {
        ...
        let err = Wallet::from_keystore(tmp.to_str().unwrap(), "x").expect_err("must reject v2");
    ...
    fn pba_l4_010_from_env_removes_the_secret_from_the_environment() {
        ...
        let _env = crate::outbound::ENV_TEST_LOCK
            .lock()
